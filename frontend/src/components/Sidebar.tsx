...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z" />
      </svg>
    ),
  },
  {
...
const ROLE_COLORS: Record<string, string> = {
  ...
  engineer: "text-purple-400 bg-purple-400/10",
  auditor: "text-emerald-400 bg-emerald-400/10",
  contractor: "text-amber-400 bg-amber-400/10",
};
    ...
    <>
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-border bg-card sticky top-0 z-40">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-foreground">ProjectPulse</span>
        </div>
      ...
      {/* Sidebar Container */}
      <aside 
        className={`fixed md:sticky top-0 left-0 z-50 h-screen bg-card border-r border-border shrink-0 flex flex-col transition-all duration-300 print:hidden
      ...
      >
        ...
        <button 
          ...
          className="hidden md:flex absolute -right-3 top-6 w-6 h-6 bg-card border border-border rounded-full items-center justify-center text-muted-foreground hover:text-foreground hover:border-blue-500 shadow-sm z-10 transition-colors"
        >
        ...
        {/* Logo Section */}
        <div className={`px-5 py-5 border-b border-border flex items-center ${isCollapsed ? "justify-center px-0" : "gap-2.5"}`}>
          <div className="w-8 h-8 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center justify-center shrink-0">
             <svg className="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" />
             </svg>
          ...
          {!isCollapsed && (
            <div className="min-w-0">
              <p className="text-sm font-bold text-foreground leading-tight truncate">ProjectPulse</p>
              <p className="text-[10px] text-muted-foreground leading-tight truncate">EPC Intelligence</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto overflow-x-hidden">
          <p className={`text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2 ${isCollapsed ? "text-center px-0" : "px-2"}`}>
          ...
          {NAV_ITEMS.filter(item => !item.roles || (user && item.roles.includes(user.role))).map((item) => (
            <Link
              ...
              href={item.href}
              onClick={() => setIsMobileOpen(false)}
              className={`nav-item ${pathname === item.href || pathname.startsWith(item.href + "/") ? "active" : ""} ${isCollapsed ? "justify-center px-0" : ""}`}
        ...
        {user && (
          <div className="px-3 py-4 border-t border-border">
            <div className={`flex items-center rounded-lg bg-secondary/50 ${isCollapsed ? "justify-center p-2" : "gap-2.5 px-2 py-2"}`}>
